...
hostnames, interface names, or device IDs. Some files are notes, examples, or
stored data rather than runnable scripts.
...
| Area | Path | Purpose |
|:-----|:-----|:--------|
| Backups | [`backup.sh`](backup.sh) | rsync backup for the hard-coded `/mnt/backup_usb/prodesk/` layout. |
| Backups | [`backup-full.sh`](backup-full.sh) | Prompted rsync backup flow for the hard-coded `/mnt/backup_usb/prodesk-full/` layout. |
...
| File | Notes |
|:-----|:------|
| [`backup.sh`](backup.sh) | Writes a package list, stops all Docker containers, rsyncs `/` to `/mnt/backup_usb/prodesk/`, then starts containers with `exited` status. |
...
| [`sys_logs/net_report.sh`](sys_logs/net_report.sh) | Runs `/usr/bin/speedtest --csv` and stores results in `/home/sys_logs/logs/system.db`. |
| [`sys_logs/temp_report.sh`](sys_logs/temp_report.sh) | Logs Melbourne weather from `wttr.in`, CPU temp from `sensors`, and optional NVIDIA GPU data to SQLite. |
...
| [`sys_logs/logs/system.db`](sys_logs/logs/system.db) | Checked-in SQLite database file used by the logging scripts. |
...
`/home/sys_logs`, `/etc/letsencrypt/live/DOMAIN_NAME`, and pfSense's
